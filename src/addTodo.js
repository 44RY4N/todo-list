...
function addTodo(todo, pushToItems = true) {
   ...
   if( pushToItems){
    console.log(`state.currentProject.length = ${state.currentProject.items.length}`);
 ...
 if (!opts.classList.contains("disabled")) { // Check if opts is enabled
...
if (todo.checked) {
    ...
    state.currentProject.items = state.currentProject.items.filter(t => t.id !== todo.id);
    // Remove ittt
    todoElement.remove();

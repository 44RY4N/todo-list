...
createButton.addEventListener("click", () => {
    ...
    if (editTodoId !== null) {
        ...
        const oldTodo = state.currentProject?.items.find(t => t.id === editTodoId);
        const oldTodoElement = document.querySelector(`[data-todo-id="${editTodoId}"]`);
        ...
        if (oldTodo && oldTodoElement) {
            ...
            console.error("Failed to find old todo or element for deletion:", { oldTodo, oldTodoElement });
        }
        editTodoId = null;
        createButton.textContent = "Add Todo";
    } else {
...
function editTodo(todo) {
    ...
    document.querySelector("#popTodoDuedate").value = todo.duedate !== "no duedate" && !isNaN(todo.duedate) ? new Date(todo.duedate).toISOString().split("T")[0] : "";
